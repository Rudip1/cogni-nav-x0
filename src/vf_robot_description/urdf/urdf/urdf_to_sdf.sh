...
# urdf_to_sdf.sh
# ───────────────
# Converts plain URDF → SDF for Gazebo simulation.
#
# Run from inside the urdf/urdf/ folder:
#   ./urdf_to_sdf.sh uvc1_virofighter.urdf
#
# Folder depth (NEW layout):
#   urdf/urdf/ → urdf/ → vf_robot_description/ → src/ → cogni-nav-x0/
#   4 levels up to workspace root  (was 3 in old flat urdf/ layout)
#
# Pipeline:
#   Step 1 — Xacro resolution  (if xmlns:xacro namespace detected)
#   Step 2 — URDF validation   (check_urdf)
#   Step 3 — URDF → raw SDF   (gz sdf -p)
#   Step 4 — Post-processing:
#             4a. Fix mesh URIs  (model://anything → model://uvc1_common)
#             4b. Auto-extract ALL <gazebo> plugins from URDF
...
#   Edit plugins in uvc1_virofighter.urdf → re-run → SDF always in sync.
...
usage() {
    echo ""
    echo -e "  ${CYAN}Usage:${NC}   ./urdf_to_sdf.sh <urdf_filename>"
    echo -e "  ${CYAN}Example:${NC} ./urdf_to_sdf.sh uvc1_virofighter.urdf"
    echo ""
    echo "  Run from inside the urdf/urdf/ directory."
    echo ""
    exit 1
}

[ -z "$1" ] && { warn "No URDF filename provided."; usage; }

# ── paths ─────────────────────────────────────────────────────────────────────
# urdf/urdf/urdf_to_sdf.sh
# depth: urdf/urdf → urdf → vf_robot_description → src → cogni-nav-x0
#                    ^^^^   ^^^^^^^^^^^^^^^^^^^^   ^^^   ^^^^^^^^^^^^
#                    +1          +2                +3       +4
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
URDF_SRC="${SCRIPT_DIR}/$1"
...
check_and_install "xacro"      "ros-humble-xacro" "xacro"      || XACRO_MISSING=1
...
[ -f "${URDF_SRC}" ]     || error "URDF not found: ${URDF_SRC}"
...
# ── Step 1 — Xacro resolution ─────────────────────────────────────────────────
RESOLVED_URDF="/tmp/vf_urdf_resolved_$$.urdf"
info "── Step 1 : Xacro resolution ───────────────────────"
if grep -q 'xmlns:xacro' "${URDF_SRC}" 2>/dev/null; then
    [ -n "${XACRO_MISSING}" ] && error "xacro required but not installed."
    info "xmlns:xacro detected — resolving…"
    xacro "${URDF_SRC}" -o "${RESOLVED_URDF}"
    info "Xacro resolved ✓"
else
    cp "${URDF_SRC}" "${RESOLVED_URDF}"
    info "Plain URDF — no xacro needed ✓"
fi
echo ""
...
# ── Step 3 — URDF → raw SDF ──────────────────────────────────────────────────
TEMP_SDF="/tmp/vf_urdf_raw_$$.sdf"
info "── Step 3 : Converting URDF → SDF ─────────────────"
...
# ── Step 4 — Post-processing ──────────────────────────────────────────────────
PATCHED_SDF="/tmp/vf_urdf_patched_$$.sdf"
info "── Step 4 : Post-processing SDF ───────────────────"
info "  4a. Fix mesh URIs        → model://${MODEL_NAME}/meshes/..."
info "  4b. Extract plugins from URDF <gazebo> tags"
...
try:
    ...
    plugin_lines = [
        "\n    <!-- ==========================================",
        "         AUTO-EXTRACTED FROM URDF <gazebo> TAGS",
        "         Source: urdf/urdf/urdf_to_sdf.sh",
        "         Do not edit model.sdf manually.",
        "         Edit URDF and re-run urdf_to_sdf.sh",
        "         ========================================== -->\n"
    ]
    extracted = 0
    ...
    print(f"[ERROR] Failed to parse URDF: {e}", file=sys.stderr); sys.exit(1)
...
info ""
info "   Workflow:"
info "   1. Edit plugins in uvc1_virofighter.urdf"
info "   2. cd urdf/urdf && ./urdf_to_sdf.sh uvc1_virofighter.urdf"
