...
─────────────────────────────────────────────────────────────────────────────
  ...
  python3 training/train.py --data-dir training/data --epochs 100

─────────────────────────────────────────────────────────────────────────────
METHOD: META_CRITIC (novel contribution — ideal weight recovery)
─────────────────────────────────────────────────────────────────────────────
  ...
  Output file   : meta_critic_best.pt
...
def compute_loss(model, features, labels):
    """
    META_CRITIC loss: MSE between predicted and ideal weight vectors.
    """
...
def compute_val_metric(outputs, labels):
    """
    Validation metric: mean cosine similarity between predicted and ideal weights.
...
def train(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for features, labels in train_loader:
            ...
            if torch.isnan(loss):
                print(f"[META_CRITIC] WARNING: NaN loss in training batch, skipping")
                continue
    ...
    print(f"  cp {CKPT_DIR}/meta_critic_best.pt meta_critic/models/meta_critic.pt")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train MetaCriticMLP (META_CRITIC method)",
    ...
    # Keep --method for compatibility but ignore it
    parser.add_argument(
        "--method", default="META_CRITIC", help="Ignored, always META_CRITIC"
    )
